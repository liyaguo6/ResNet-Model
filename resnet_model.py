...
class ResNet(object):
  ...
  def _build_model(self):
    with tf.variable_scope('init'):
      # x shape:[batch_size,height,weight,channel]
      x = self._images
      """第一层卷积（3,3x3/1,16）"""
    ...
    with tf.variable_scope('unit_last'):
      x = self._batch_norm('final_bn', x) # shape = （128，8，8，64）
      x = self._relu(x, self.hps.relu_leakiness) # shape = （128，8，8，64）
      x = self._global_avg_pool(x)   #shape = (128,64)
