...
def train(hps):
  ...
  images, labels = read_tfrecord.build_input(
      data_path=FLAGS.train_data_path, batch_size=hps.batch_size, resize=32,dtype='float',num_class=hps.num_classes)
  # 构建残差网络模型
...
def evaluate(hps):
  # 构建输入数据(读取队列执行器）
  images, labels = read_tfrecord.build_input(
      data_path=FLAGS.train_data_path, batch_size=hps.batch_size, resize=32,
      dtype='float32', num_class=hps.num_classes)
  # 构建残差网络模型
  model = resnet_model.ResNet(hps, images, labels, FLAGS.mode)
  model.build_graph()
  # 模型变量存储器
